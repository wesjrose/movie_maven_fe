        ...
        >
          {items.map((person) => (
            <ListboxOption
              key={person.id}
              value={person}
              className="group relative cursor-default py-2 pr-12 pl-3 text-black select-none data-focus:bg-[var(--blue-selected)] data-focus:outline-hidden"
            >
              <div className="flex items-center">
                <span className="ml-3 block truncate font-normal group-data-selected:font-normal">
                  {person.name}
                </span>
              </div>

              <span className="absolute inset-y-0 right-0 flex items-center pr-4 text-[var(--blue-selected)] group-not-data-selected:hidden group-data-focus:text-white">
